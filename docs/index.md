...
devices: compact UI snapshots, stable action refs, app diagnostics, artifacts, file transfer, and
JVM debugging, and replayable trace archives from one CLI.
...
`daemon` `device` `session` `trace` `ui` `action` `wait` `app` `artifact` `emulator` `reliability`
`file` `debug`
